...
class TriBranchDTI(nn.Module):
    ...
    def forward(self, prot_data, lig_data):
        ...
        # Fusion stage 2
        h = torch.cat([prot_emb, lig_emb, attr_emb], dim=1)
...
class DTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return prot_graph, lig_graph, y, w
        except Exception:
            return None
...
def train():
    ...
    batch_size = 30  
    train_dataset = DTIDataset(train_df, BASE_PATH)
    ...
    if train_mse_hist and val_mse_hist:
        plot_mse_curve(train_mse_hist, val_mse_hist, save_path="/home/ubuntu/mol_gen_project/Feb20/mse_loss_curve.png")
    ...
    for name, loader in [("Train", train_loader), ("Validation", val_loader), ("Test", test_loader)]:
        ...
        if name == "Test":
            plot_pred_vs_true(yt, yp, save_path="/home/ubuntu/mol_gen_project/Feb20/pred_vs_true_test.png")

    if results:
        dfm = pd.DataFrame(results).set_index("Split")
        dfm.to_csv("final_metrics.csv")

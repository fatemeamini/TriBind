...
from protein_parser_noply import protein_to_graph
...
class DTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if sdf_path is None or pdb_path is None:
            return None
        ...
        try:
            ...
            prot_graph = protein_to_graph(None, pdb_path)
...
# 5) Plots
# ================================================================
def plot_mse_curve(train_mse, val_mse, save_path="/home/ubuntu/mol_gen_project/Apr23/mse_loss_curve.png"):
...
def plot_pred_vs_true(y_true, y_pred, save_path="/home/ubuntu/mol_gen_project/Apr23/pred_vs_true_test.png", title="Pred vs True (Test)"):
...
class StratifiedBatchSampler(Sampler):
    ...
    def __init__(self, labels, batch_size, shuffle=True, drop_last=False, generator=None):
        assert batch_size % 3 == 0, "Batch size must be divisible by 3 for stratified sampling."
...
def train():
    ...
    BASE_PATH = "/home/ubuntu/mol_gen_project/MolGen_Project copy/data/dataset/PDBbind_general"
    ...
    scaler_path = os.path.join(os.path.dirname(__file__), "/home/ubuntu/mol_gen_project/Apr23/target_scaler_params.json")
    ...
    p, l, inter, y, w = sample

    print("Protein node dim:", p.x.shape[1])
    print("Ligand node dim:", l.x.shape[1])
    print("Interaction node dim:", inter.x.shape[1])

    model = TriBranchDTI(
        prot_node_dim=p.x.shape[1],
        lig_node_dim=l.x.shape[1],
        int_node_dim=inter.x.shape[1]
    ).to(device)
    ...
    if train_mse_hist and val_mse_hist:
        plot_mse_curve(train_mse_hist, val_mse_hist, save_path="/home/ubuntu/mol_gen_project/Apr23/mse_loss_curve.png")

    # Final evaluation from best checkpoint
    if os.path.exists("best_model.pth"):
        model.load_state_dict(torch.load("/home/ubuntu/mol_gen_project/Apr23/best_model.pth", map_location=device))
        model.eval()
    ...
    for name, loader in [("Train", train_loader), ("Validation", val_loader), ("Test", test_loader)]:
        ...
        if name == "Test":
            plot_pred_vs_true(yt, yp, save_path="/home/ubuntu/mol_gen_project/Apr23/pred_vs_true_test.png")

    if results:
        dfm = pd.DataFrame(results).set_index("Split")
        dfm.to_csv("/home/ubuntu/mol_gen_project/Apr23/final_metrics.csv")
